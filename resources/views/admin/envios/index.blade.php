...
<div
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">Supervisa transportadoras, entregas a domicilio y guías de transporte a nivel nacional</p>
        </div>

        <div class="flex items-center gap-2">
            <a href="{{ route('admin.pedidos.index') }}" class="px-4 py-2.5 bg-white dark:bg-slate-800 hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-800 dark:text-slate-200 font-bold text-xs rounded-xl border border-gray-200 dark:border-slate-700 shadow-2xs transition flex items-center gap-2">
    ...
    {{-- ==================== MENSAJES FLASH ==================== --}}
    @if(session('success'))
        <div class="px-4 py-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-700 dark:text-emerald-300 rounded-2xl flex items-center justify-between text-xs font-bold shadow-xs">
            <div class="flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-gray-200/80 dark:border-slate-800 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-xs font-bold text-gray-400 dark:text-slate-400 block uppercase">Total Envíos</span>
                <span class="text-2xl font-black text-gray-900 dark:text-white mt-0.5 block">{{ $totalEnvios }}</span>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-violet-50 dark:bg-violet-950/50 text-[#7c3aed] dark:text-violet-400 flex items-center justify-center text-xl font-bold border border-violet-100 dark:border-violet-900/50">
                🚚
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-gray-200/80 dark:border-slate-800 shadow-xs flex items-center justify-between">
            <div>
                ...
                <span class="text-2xl font-black text-gray-900 dark:text-white mt-0.5 block">{{ $enviosPendientes }}</span>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-950/50 text-amber-600 dark:text-amber-400 flex items-center justify-center text-xl font-bold border border-amber-100 dark:border-amber-900/50">
                📦
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-gray-200/80 dark:border-slate-800 shadow-xs flex items-center justify-between">
            <div>
                ...
                <span class="text-2xl font-black text-gray-900 dark:text-white mt-0.5 block">{{ $enviosEnCamino }}</span>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xl font-bold border border-blue-100 dark:border-blue-900/50">
                🛣️
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-gray-200/80 dark:border-slate-800 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-xs font-bold text-emerald-600 dark:text-emerald-400 block uppercase">Entregados</span>
                <span class="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-0.5 block">{{ $enviosEntregados }}</span>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xl font-bold border border-emerald-100 dark:border-emerald-900/50">
                ✅
            </div>
        </div>
        ...
        <form method="GET" action="{{ route('admin.envios.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center text-xs">
            ...
            <div class="sm:col-span-5 relative">
                ...
                <input 
                    ...
                    class="w-full pl-9 pr-3.5 py-2.5 rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 text-xs focus:ring-1 focus:ring-[#7c3aed]">
            </div>

            {{-- Filtro Estado Envío --}}
            <div class="sm:col-span-3">
                <select name="estado" class="w-full rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white px-3 py-2.5 text-xs focus:ring-1 focus:ring-[#7c3aed]">
            ...
            <div class="sm:col-span-2">
                <select name="empresa" class="w-full rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white px-3 py-2.5 text-xs focus:ring-1 focus:ring-[#7c3aed]">
            ...
            <div class="sm:col-span-2 flex items-center gap-2">
                <button 
                    type="submit" 
                    class="flex-1 py-2.5 px-4 bg-[#0f172a] dark:bg-violet-600 hover:bg-black dark:hover:bg-violet-500 text-white font-bold text-xs rounded-xl transition shadow-xs">
                    Filtrar
                </button>
                @if(request()->hasAny(['q', 'estado', 'empresa']))
                    <a href="{{ route('admin.envios.index') }}" class="p-2.5 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-600 dark:text-slate-300 rounded-xl transition font-bold" title="Limpiar filtros">
                        ...
                        <tr class="hover:bg-gray-50/60 dark:hover:bg-slate-800/40 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-xl bg-violet-50 dark:bg-violet-950/50 text-[#7c3aed] dark:text-violet-400 flex items-center justify-center font-bold text-sm shrink-0 border border-violet-100 dark:border-violet-900/50">
                                        🚚
                                    </div>
                                    <div>
                                        <span class="font-black text-gray-950 dark:text-white text-sm block">Envío #{{ $envio->id }}</span>
                                        <span class="text-[10px] text-gray-400 dark:text-slate-400 font-bold block">Pedido #{{ $envio->pedido_id }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    <span class="font-bold text-gray-900 dark:text-slate-200 block text-xs">
                            ...
                            <td class="px-6 py-4 max-w-xs">
                                <div class="flex items-start gap-1.5">
                                    ...
                                    <span class="text-xs text-gray-700 dark:text-slate-300 font-medium line-clamp-2 leading-relaxed">
                            ...
                            {{-- Transportadora --}}
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 rounded-lg bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-200 font-black text-[11px] inline-flex items-center gap-1 border border-gray-200 dark:border-slate-700">
                            ...
                            <td class="px-6 py-4 font-bold text-xs">
                                @if($envio->costo > 0)
                                    <span class="text-gray-900 dark:text-white">${{ number_format($envio->costo, 0, ',', '.') }}</span>
                                @else
                                    <span class="text-emerald-600 dark:text-emerald-400 font-black">GRATIS</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @if(str_contains($est, 'cancel') || str_contains($est, 'devuel'))
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 border border-rose-200/80 dark:border-rose-800/60 shadow-2xs whitespace-nowrap">
                                        <span class="w-1.5 h-1.5 rounded-full bg-rose-500 shrink-0"></span>
                                        Cancelado
                                    </span>
                                @elseif(str_contains($est, 'enviado') || str_contains($est, 'camino') || str_contains($est, 'tránsito'))
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-200/80 dark:border-blue-800/60 shadow-2xs whitespace-nowrap">
                                        <span class="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0"></span>
                                        Enviado
                                    </span>
                                @elseif(str_contains($est, 'entreg'))
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200/80 dark:border-emerald-800/60 shadow-2xs whitespace-nowrap">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0"></span>
                                        Entregado
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200/80 dark:border-amber-800/60 shadow-2xs whitespace-nowrap">
                            ...
                            {{-- Fecha --}}
                            <td class="px-6 py-4 text-[11px] text-gray-500 dark:text-slate-400">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <button
                                        ...
                                        class="px-2.5 py-1.5 rounded-lg bg-gray-100 dark:bg-slate-800 hover:bg-violet-100 dark:hover:bg-violet-900/50 text-gray-700 dark:text-slate-200 hover:text-violet-900 dark:hover:text-violet-300 font-bold text-xs transition inline-flex items-center gap-1 cursor-pointer border border-transparent dark:border-slate-700"
                                    ...
                                    <form action="{{ route('admin.envios.destroy', $envio->id) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas cancelar este envío #{{ $envio->id }}?')">
                                        ...
                                        <button
                                            type="submit"
                                            class="w-7 h-7 rounded-lg bg-gray-100 dark:bg-slate-800 hover:bg-red-100 dark:hover:bg-red-900/50 text-gray-500 dark:text-slate-400 hover:text-red-700 dark:hover:text-red-400 flex items-center justify-center transition"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-gray-400 dark:text-slate-500">
                                <i class="fa-solid fa-truck-fast text-3xl mb-2 text-gray-300 dark:text-slate-600"></i>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-7 max-w-md w-full shadow-2xl border border-gray-100 dark:border-slate-800 relative">
            <div class="flex items-center justify-between pb-3 mb-5 border-b border-gray-100 dark:border-slate-800">
                <h3 class="text-sm font-black text-gray-900 dark:text-white flex items-center gap-2">
                    <span>🚚</span>
                    <span>Actualizar Despacho <strong class="text-[#7c3aed] dark:text-violet-400" x-text="editData ? '#' + editData.id : ''"></strong></span>
                </h3>
                <button type="button" @click="modalEditar = false" class="text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-slate-200">
                    <i class="fa-solid fa-xmark text-sm"></i>
                </button>
            </div>
            ...
            <form :action="editUrl" method="POST" class="space-y-4 text-xs">
                ...
                <div>
                    ...
                    <select name="empresa_envios" x-model="editData.empresa_envios" class="w-full rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white p-2.5 text-xs font-bold focus:ring-1 focus:ring-[#7c3aed]">
                ...
                {{-- Estado del Envío --}}
                <div>
                    <label class="block font-bold text-gray-700 dark:text-slate-300 uppercase mb-1">Estado del Envío *</label>
                    <select name="estado" x-model="editData.estado" class="w-full rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white p-2.5 text-xs font-bold focus:ring-1 focus:ring-[#7c3aed]">
                ...
                {{-- Costo de Envío --}}
                <div>
                    <label class="block font-bold text-gray-700 dark:text-slate-300 uppercase mb-1">Costo de Envío ($ COP)</label>
                    <input type="number" step="0.01" name="costo" x-model="editData.costo" class="w-full rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white p-2.5 text-xs">
                </div>

                {{-- Dirección de Entrega --}}
                <div>
                    <label class="block font-bold text-gray-700 dark:text-slate-300 uppercase mb-1">Dirección de Entrega *</label>
                    <input type="text" name="direccion_envio" x-model="editData.direccion_envio" required class="w-full rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white p-2.5 text-xs">
                </div>

                <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100 dark:border-slate-800">
                    <button type="button" @click="modalEditar = false" class="px-4 py-2 rounded-xl border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-slate-300 font-bold text-xs hover:bg-gray-50 dark:hover:bg-slate-800">
                        Cancelar
                    </button>
                    <button type="submit" class="px-5 py-2 rounded-xl bg-[#0f172a] dark:bg-violet-600 hover:bg-black dark:hover:bg-violet-500 text-white font-bold text-xs shadow-xs">
                        Guardar Despacho
                    </button>
                </div>
            </form>
    ...
    <div 
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-md w-full shadow-2xl border border-gray-100 dark:border-slate-800 relative my-8" @click.stop x-show="envioActivo">
            
            {{-- Encabezado Modal --}}
            <div class="flex items-center justify-between pb-4 mb-4 border-b border-gray-100 dark:border-slate-800 no-imprimir">
                <div class="flex items-center gap-2">
                    <span class="text-lg">🏷️</span>
                    <div>
                        <h3 class="text-xs font-black text-gray-900 dark:text-white">Rótulo de Despacho</h3>
                        <p class="text-[10px] text-gray-400 dark:text-slate-400">Tirilla térmica 80mm para empaque</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2">
                    <button 
                        ...
                        class="px-3 py-1.5 bg-[#0f172a] dark:bg-violet-600 hover:bg-black dark:hover:bg-violet-500 text-white font-bold text-xs rounded-xl transition flex items-center gap-1.5 shadow-xs cursor-pointer">
                    ...
                    <button 
                        ...
                        class="w-8 h-8 rounded-xl bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-500 dark:text-slate-300 flex items-center justify-center transition cursor-pointer">
                        <i class="fa-solid fa-xmark text-sm"></i>
                    </button>
                </div>
            </div>
